...
{
    public partial class formtblKhachHang : Form
    {
        
        
        Repos.repo_tblKhachHang repo = new Repos.repo_tblKhachHang();
        DataTable source;
        ...
        List<Tuple<myEvents, tblKhachHang>> userEvent = new List<Tuple<myEvents, tblKhachHang>>();
        
        public formtblKhachHang()
        {
            InitializeComponent();
        }
        ...
        public void MakeEmptyTextbox()
        {
			...
			this.numbid.Value = 0;
			this.txttenKhachHang.Text = null;
			this.txtdiaChi.Text = null;
			this.txtdienThoai.Text = null;
        ...
        public void DataGridView_To_Textbox(int index)
        {
            try
            {
                ...
                if(tenKhachHang != null)
                    this.txttenKhachHang.Text = tenKhachHang.ToString();
                ...
                if(dienThoai != null)
                    this.txtdienThoai.Text = dienThoai.ToString();
        ...
        {
			FunctionHelper.ExportExcel(ref this.dgvData, "tblKhachHang");
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var record = Textbox_To_tblKhachHang();
                var result = repo.Delete(record);

                if (result)
                {
                    ShowMessage("Xoá bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, tblKhachHang>(myEvents.Delete, record));
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var result = repo.Update(Textbox_To_tblKhachHang());

                if (result)
                {
                    ShowMessage("Cập nhật bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, tblKhachHang>(myEvents.Update, DataGridView_To_tblKhachHang(dgvData.SelectedRows[0].Index)));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    userEvent.Add(new Tuple<myEvents, tblKhachHang>(myEvents.Insert, record));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        {
			source = FunctionHelper.ToDataTable<tblKhachHang>(repo.FindAll());
        ...
        private void formtblKhachHang_Load(object sender, EventArgs e)
        {
            btnLamMoi_Click(sender, e);
        }
        ...
        private void formtblKhachHang_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (this.Owner != null)
