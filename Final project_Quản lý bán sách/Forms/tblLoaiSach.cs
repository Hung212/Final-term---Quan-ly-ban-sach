...
{
    public partial class formtblLoaiSach : Form
    {
        
        
        Repos.repo_tblLoaiSach repo = new Repos.repo_tblLoaiSach();
        DataTable source;
        ...
        List<Tuple<myEvents, tblLoaiSach>> userEvent = new List<Tuple<myEvents, tblLoaiSach>>();
        
        public formtblLoaiSach()
        {
            InitializeComponent();
        }
        ...
        // Chuyển giá trị trong Textbox thành tblLoaiSach
        public tblLoaiSach Textbox_To_tblLoaiSach()
        {
            try
            {
            ...
            result.tenLoai = (this.txttenLoai.Text == null ? null : (string)(this.txttenLoai.Text));
        ...
        public void DataGridView_To_Textbox(int index)
        {
            try
            {
                ...
                var tenLoai = this.dgvData.Rows[index].Cells["tenLoai"].Value;
                if(tenLoai != null)
                    this.txttenLoai.Text = tenLoai.ToString();
            }
            catch (Exception ex) { ShowMessage(ex.Message, false); } // Không load được hoặc xảy ra lỗi
        }

        // Chuyển giá trị trong DataGridView thành tblLoaiSach
        public tblLoaiSach DataGridView_To_tblLoaiSach(int index)
        {
            try
            {
                tblLoaiSach result = new tblLoaiSach();
                ...
                var tenLoai = this.dgvData.Rows[index].Cells["tenLoai"].Value;
                if(tenLoai != null)
                    result.tenLoai = (tenLoai == null ? null : (string)(tenLoai));
        ...
        {
			FunctionHelper.ExportExcel(ref this.dgvData, "tblLoaiSach");
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var record = Textbox_To_tblLoaiSach();
                var result = repo.Delete(record);

                if (result)
                {
                    ShowMessage("Xoá bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, tblLoaiSach>(myEvents.Delete, record));
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var result = repo.Update(Textbox_To_tblLoaiSach());

                if (result)
                {
                    ShowMessage("Cập nhật bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, tblLoaiSach>(myEvents.Update, DataGridView_To_tblLoaiSach(dgvData.SelectedRows[0].Index)));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    userEvent.Add(new Tuple<myEvents, tblLoaiSach>(myEvents.Insert, record));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        {
			source = FunctionHelper.ToDataTable<tblLoaiSach>(repo.FindAll());
        ...
        private void formtblLoaiSach_Load(object sender, EventArgs e)
        {
            btnLamMoi_Click(sender, e);
        }
        ...
        private void formtblLoaiSach_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (this.Owner != null)
