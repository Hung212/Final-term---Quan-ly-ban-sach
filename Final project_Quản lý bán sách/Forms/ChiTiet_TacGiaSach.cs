...
{
    public partial class formChiTiet_TacGiaSach : Form
    {
        
        
        Repos.repo_ChiTiet_TacGiaSach repo = new Repos.repo_ChiTiet_TacGiaSach();
        DataTable source;
        ...
        List<Tuple<myEvents, ChiTiet_TacGiaSach>> userEvent = new List<Tuple<myEvents, ChiTiet_TacGiaSach>>();
        
        public formChiTiet_TacGiaSach()
        {
            InitializeComponent();
        }
        ...
        // Chuyển giá trị trong Textbox thành ChiTiet_TacGiaSach
        public ChiTiet_TacGiaSach Textbox_To_ChiTiet_TacGiaSach()
        {
            try
            {
                ...
                result.id_tblTacGia = (int)this.numbid_tblTacGia.Value;
            result.vaiTro = (this.txtvaiTro.Text == null ? null : (string)(this.txtvaiTro.Text));
        ...
        public void MakeEmptyTextbox()
        {
			...
			this.numbid_tblTacGia.Value = 0;
			this.txtvaiTro.Text = null;
        }
        ...
        public void DataGridView_To_Textbox(int index)
        {
            try
            {
                ...
                var id_tblTacGia = this.dgvData.Rows[index].Cells["id_tblTacGia"].Value;
                if(id_tblTacGia != null)
                    this.numbid_tblTacGia.Value = id_tblTacGia != null ? decimal.Parse(id_tblTacGia.ToString()) : 0;
                
                var vaiTro = this.dgvData.Rows[index].Cells["vaiTro"].Value;
                if(vaiTro != null)
                    this.txtvaiTro.Text = vaiTro.ToString();
            }
            catch (Exception ex) { ShowMessage(ex.Message, false); } // Không load được hoặc xảy ra lỗi
        }

        // Chuyển giá trị trong DataGridView thành ChiTiet_TacGiaSach
        public ChiTiet_TacGiaSach DataGridView_To_ChiTiet_TacGiaSach(int index)
        {
            try
            {
                ChiTiet_TacGiaSach result = new ChiTiet_TacGiaSach();
                ...
                var id_tblTacGia = this.dgvData.Rows[index].Cells["id_tblTacGia"].Value;
                if(id_tblTacGia != null)
                    result.id_tblTacGia = (id_tblTacGia == null ? null : (int?)int.Parse(id_tblTacGia.ToString()));
                
                var vaiTro = this.dgvData.Rows[index].Cells["vaiTro"].Value;
                if(vaiTro != null)
                    result.vaiTro = (vaiTro == null ? null : (string)(vaiTro));
        ...
        {
			FunctionHelper.ExportExcel(ref this.dgvData, "ChiTiet_TacGiaSach");
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var record = Textbox_To_ChiTiet_TacGiaSach();
                var result = repo.Delete(record);

                if (result)
                {
                    ShowMessage("Xoá bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, ChiTiet_TacGiaSach>(myEvents.Delete, record));
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var result = repo.Update(Textbox_To_ChiTiet_TacGiaSach());

                if (result)
                {
                    ShowMessage("Cập nhật bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, ChiTiet_TacGiaSach>(myEvents.Update, DataGridView_To_ChiTiet_TacGiaSach(dgvData.SelectedRows[0].Index)));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    userEvent.Add(new Tuple<myEvents, ChiTiet_TacGiaSach>(myEvents.Insert, record));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        {
			source = FunctionHelper.ToDataTable<ChiTiet_TacGiaSach>(repo.FindAll());
        ...
        private void formChiTiet_TacGiaSach_Load(object sender, EventArgs e)
        {
            btnLamMoi_Click(sender, e);
        }
        ...
        private void formChiTiet_TacGiaSach_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (this.Owner != null)
        ...
        bool bfid_tblTacGia = false;
        private void btnfid_tblTacGia_Click(object sender, EventArgs e)
        {
            this.Enabled = false;
            bfid_tblTacGia = true;
            var form = new fSearchAndReturn("select * from tblTacGia", "id");
            form.Owner = this;
            form.Show();
        }
        
        
        private void formChiTiet_TacGiaSach_EnabledChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (bfid_tblTacGia)
                {
                    bfid_tblTacGia = false;
                    if(this.Tag == null)
                        return;
                    this.numbid_tblTacGia.Value = int.Parse((this.Tag as object[])[0].ToString());
                    this.Tag = null;
                    return;
                }
            }
