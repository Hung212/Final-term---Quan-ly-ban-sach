...
{
    public partial class formtblHoaDon : Form
    {
        
        
        Repos.repo_tblHoaDon repo = new Repos.repo_tblHoaDon();
        DataTable source;
        ...
        List<Tuple<myEvents, tblHoaDon>> userEvent = new List<Tuple<myEvents, tblHoaDon>>();
        
        public formtblHoaDon()
        {
            InitializeComponent();
        }
        ...
        public void MakeEmptyTextbox()
        {
			...
			this.numbid.Value = 0;
			this.datengayTao.Value = DateTime.Now;
			this.txttenNhanVien.Text = null;
			this.numbtongTien.Value = 0;
        }
        ...
        public void DataGridView_To_Textbox(int index)
        {
            try
            {
                ...
                if(ngayTao != null)
                    this.datengayTao.Value = ngayTao != null ? DateTime.Parse(ngayTao.ToString()) : DateTime.Now;
                
                var tenNhanVien = this.dgvData.Rows[index].Cells["tenNhanVien"].Value;
                if(tenNhanVien != null)
                    this.txttenNhanVien.Text = tenNhanVien.ToString();
                
                var tongTien = this.dgvData.Rows[index].Cells["tongTien"].Value;
                if(tongTien != null)
                    this.numbtongTien.Value = tongTien != null ? decimal.Parse(tongTien.ToString()) : 0;
            }
            catch (Exception ex) { ShowMessage(ex.Message, false); } // Không load được hoặc xảy ra lỗi
        }
        ...
        {
			FunctionHelper.ExportExcel(ref this.dgvData, "tblHoaDon");
        }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var record = Textbox_To_tblHoaDon();
                var result = repo.Delete(record);

                if (result)
                {
                    ShowMessage("Xoá bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, tblHoaDon>(myEvents.Delete, record));
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var result = repo.Update(Textbox_To_tblHoaDon());

                if (result)
                {
                    ShowMessage("Cập nhật bản ghi thành công!", result);
                    userEvent.Add(new Tuple<myEvents, tblHoaDon>(myEvents.Update, DataGridView_To_tblHoaDon(dgvData.SelectedRows[0].Index)));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    userEvent.Add(new Tuple<myEvents, tblHoaDon>(myEvents.Insert, record));
                    btnLamMoi_Click(sender, e);
                }
                else
                {
        ...
        {
			source = FunctionHelper.ToDataTable<tblHoaDon>(repo.FindAll());
        ...
        private void formtblHoaDon_Load(object sender, EventArgs e)
        {
            btnLamMoi_Click(sender, e);
        }
        ...
        private void formtblHoaDon_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (this.Owner != null)
